...
function BoardContent({ board, createNewColumn, createNewCard, moveColumn, moveCardInTheSameColumn, moveCardToDifferentColumn, deleteColumnDetails }) {
  ...
  useEffect(() => {
    //Columns đã được sắp xếp ở component cha cao nhất (video 71)
  ...
  // Khởi tạo function chung cập nhật lại state trong trường hợp di chuyển card giữa các column khác nhau
  const moveCardBetweenDifferentColumns = (
    ...
    setOrderedColumns(prevColumns => {
      ...
      // Clone mảng OrderedColumnState cũ ra một cái mới để xử lý data rồi return - cập nhật lại OrderedColumnsState mới
      ...
      if (nextOverColumn) {
        ...
        //Xóa cái place holder card đi nếu nó đang tồn tại (video 37.2)
        nextOverColumn.cards = nextOverColumn.cards.filter(card => !card.FE_PlaceHolderCard)

        // Cập nhật lại mảng cardOrderIDs cho chuẩn dữ liệu
        nextOverColumn.cardOrderIds = nextOverColumn.cards.map(card => card._id)
      }
      //Nếu function này được gọi từ handleDragEnd nghĩa là đã kéo thả xong, lúc này mới xử lý gọi API một lần ở đây
      if (triggerFrom === 'handleDragEnd') {
        /**
      * Gọi lên prop function moveCardToDifferentColumn nằm ở component cha cao nhất (Boards/_id.jsx)
  ...
  const handleDragOver = (event) => {
    ...
    // Còn nếu kéo card thì xử lý thêm để có thể kéo cảd qua lại giữa các columns
    //console.log('handleDragOver: ', event)
    const { active, over } = event

    // Kiểm tra nếu không tồn tại active hoặc over (kéo linh tinh ra ngoài container thì return luôn tránh lỗi)
  ...
  const handleDragEnd = (event) => {
    ...
    if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.CARD) {
      ...
      // Kéo card qua 2 colum khác nhau
      // Phải dùng tới activeDragItemData.columnId hoặc oldColumnWhenDraggingCard._id (set vào state từ bước handleDragStart) chứ không phải activeData trong scope handleDragEnd này vì sau khi đi qua onDragOver tới đây state của card đã bị cập nhật một lần rồi. (video 35)
      if (oldColumnWhenDraggingCard._id !== overColumn._id) {
        ...
        //Vẫn gọi update State để tránh delay hoặc flickering giao diện lúc kéo thả khi phải chờ gọi API (small trick)
        setOrderedColumns(prevColumns => {
          // Clone mảng OrderedColumnState cũ ra một cái mới để xử lý data rồi return - cập nhật lại OrderedColumnsState mới
          const nextColumns = cloneDeep(prevColumns)
        ...
        /**
       * Gọi lên prop function moveCardInTheSameColumn nằm ở component cha cao nhất (Boards/_id.jsx)
       * Lưu ý, về sau ta sẽ đưa dữ liệu Board ra ngoài Redux Global Store thì lúc này chúng ta có thể gọi luôn API ở đây là xong thay vì phải lần lượt gọi ngược lên những component cha phía bên trên. (Đối với component con nằm càng sâu thì càng khổ)
       * Với việc sử dụng Redux như vậy thì code sẽ clean hơn rất nhiều.
       */
    ...
    if (activeDragItemType === ACTIVE_DRAG_ITEM_TYPE.COLUMN) {
      ...
      if (active.id !== over.id) {
        ...
        //Vẫn gọi update State ở đây để tránh delay hoặc flickering giao diện lúc kéo thả khi cần phải chờ gọi API, phía front-end sẽ thấy column đã được kéo thả, nhưng state được update lại behind the scence có thể sẽ chậm hơn (small trick)
        setOrderedColumns(dndOrderedColumns)

        /**
       * Gọi lên prop function moveColumn nằm ở component cha cao nhất (Boards/_id.jsx)
       * Lưu ý, về sau ta sẽ đưa dữ liệu Board ra ngoài Redux Global Store thì lúc này chúng ta có thể gọi luôn API ở đây là xong thay vì phải lần lượt gọi ngược lên những component cha phía bên trên. (Đối với component con nằm càng sâu thì càng khổ)
       * Với việc sử dụng Redux như vậy thì code sẽ clean hơn rất nhiều.
       */
    ...
    // Những dữ liệu sau khi đã kéo thả này phải luôn đưa về giá trị null mặc định
  ...
  return (
    <DndContext
      // Cảm biến (đã được giải thích ở video 30)
      sensors={mySensors}
